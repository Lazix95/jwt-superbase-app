...
const supabaseKey =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJhYXdpdHJndGt4dHR4bXJlcWxuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDcwNjI1NDcsImV4cCI6MjA2MjYzODU0N30.HMBdnJ1Y34dHBiNq66MrJ0aICsA28VEFehRdxm-sEsQ";
...
export function useApi({ shouldLoadUser } = {}) {
  ...
  async function login({ email, password }) {
    try {
      ...
      localStorage.setItem(
        "token",
        JSON.stringify({ token: accessToken, savedAt })
      );
  ...
  async function refreshToken() {
    ...
    localStorage.setItem(
      "token",
      JSON.stringify({ token: newSession, savedAt })
    );
    return newSession;
  }
